...
| 2 | Completion | The ratio derived by doing snake-length/board-size, to score the lateness of the game |
...
| 1 | Free | There is a free space |
| 2 | Snake | There is part of the snake's body |
| 3 | Wall | There is occupied space (outside of the board range) |
...
## Layers
The meta feeds into two layers, of 64 neurons each, whilst the board image feeds into 2 layers of a CNN, of 32 and 64 channels respectively. The outputs of both networks are then fed through two layers of 256 ad 128 respectively.

Here is an ascii overview of the network structure:
```bash
   Meta       Board
     ↓          ↓
 64 neurons  32 channels
     ↓          ↓
 64 neurons  64 channels
      ↘        ↙ (flatten)
      256 neurons
           ↓
      128 neurons
...
```
A ReLU activation function is used for all parts of the network
...
| --- | --- | --- |
| Apple | +10 | Reward for collecting apple |
| Got closer | +0.1 | Reward getting closer to apple |
| Got further | -0.15 | Slight punishment discourages wandering |
| Died | -1 | Small death punishment, to encourage exploration |
